...
Lightweight interpreter exception: when the original task is not a Python project task and the failure is only a standard-library or interpreter-version mismatch in a one-shot validation script, do not create or repair a project `.venv` if another already-installed interpreter can complete the same validation. Preserve the failed command and error, rerun the check with the available compatible interpreter, and report the substitution. Examples include using `python3.11` for `tomllib` TOML parsing after `python3` resolves to Python 3.10 or older. If the validation imports project packages, runs project tests, installs packages, or exposes missing third-party dependencies, resume the full recovery loop.
