---
name: brightdata-cli
description: Guide for using the Bright Data CLI (`brightdata` / `bdata`) to scrape websites, search the web, manage proxy zones, and check account budget. Use this skill whenever the user wants to scrape a URL, search Google/Bing/Yandex, check their Bright Data balance or zones, or do live web data collection from the terminal. Also trigger when the user mentions brightdata, bdata, web scraping CLI, SERP API, or wants to install Bright Data skills into their coding agent.
---
...
Read [brightdata-cli-commands.md](brightdata-cli-commands.md) for the full command reference with all flags, options, and examples for every command.
...
For async jobs from `--async` scrapes:
...
| Async job timeout | Increase with `--timeout 1200` |
