...
from .compress_detect import should_compress
from .compress_validate import validate

MAX_RETRIES = 2

MODES = {
    "lite": "Light compression. Remove filler and hedging. Keep full sentences.",
    "full": "Medium compression. Drop articles/filler. Fragments OK. Preserve clarity.",
    "ultra": "High compression. Bare fragments. Use arrows and common tech abbreviations.",
    "wenyan-lite": "Light classical Chinese compression when source language is Chinese.",
    "wenyan-full": "Strong classical Chinese compression when source language is Chinese.",
    "wenyan-ultra": "Extreme classical Chinese compression when source language is Chinese.",
}
...
def _mode_for_savings(value: int) -> str:
    if value < 35:
        return "lite"
    if value < 65:
        return "full"
...
def resolve_mode() -> tuple[str, int]:
    ...
    if path.exists():
        try:
            ...
            mode = data.get("defaultMode", mode)
            savings = int(data.get("targetSavings", savings))
            if data.get("enabled") is False or mode == "off":
                mode = "lite"
                savings = min(savings, 25)
        except (OSError, ValueError, json.JSONDecodeError):
            pass
    env_mode = os.environ.get("CAVEMAN_DEFAULT_MODE")
    if env_mode in MODES:
        mode = env_mode
        savings = {"lite": 25, "full": 50, "ultra": 75}.get(mode, savings)
    env_savings = os.environ.get("CAVEMAN_TOKEN_SAVINGS")
    if env_savings:
        try:
            savings = max(0, min(90, int(env_savings.strip().removesuffix("%"))))
            mode = _mode_for_savings(savings)
        except ValueError:
            pass
...
def resolve_provider() -> str:
    env_provider = os.environ.get("CAVEMAN_PROVIDER")
    if env_provider:
    ...
    if path.exists():
        try:
            data = json.loads(path.read_text())
            return str(data.get("provider", "auto")).lower()
        except (OSError, ValueError, json.JSONDecodeError):
            pass
    return "auto"
...
def call_model(prompt: str) -> str:
    """Send a prompt to the configured local model runner.

    Provider order in ``auto`` mode:
      1. Codex CLI when installed.
      2. Anthropic SDK when ANTHROPIC_API_KEY is set.
      3. Claude CLI when installed.
        ...
        if codex_bin:
            ...
            try:
                ...
                with open(output_path, encoding="utf-8", errors="replace") as output_file:
                    return strip_llm_wrapper(output_file.read().strip())
    ...
    if provider in {"auto", "anthropic"} and api_key:
        try:
            import anthropic
            ...
            if provider == "anthropic":
                raise RuntimeError("CAVEMAN_PROVIDER=anthropic but anthropic package is not installed")
    ...
    raise RuntimeError(
        "No model provider available. Install `codex` or `claude`, set ANTHROPIC_API_KEY, "
        "or set CAVEMAN_PROVIDER=codex|claude|anthropic."
    )


def build_compress_prompt(original: str) -> str:
    mode, savings = resolve_mode()
...
INTENSITY:
- Mode: {mode}
- Target token savings: about {savings}%
